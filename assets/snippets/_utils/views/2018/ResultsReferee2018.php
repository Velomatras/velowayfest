...
class ResultsReferee2018 {

    private $ref_voting_db_name = 'way_voting_referee_2018';
    private $db_table_name = 'way2018';
...
// выводим очередную строку таблицы
                ?> <tr> <?php
					...
					<?php
                    print_rates(Array($resultRate ? '<span style="color: red">'.sprintf("%.2f", $resultRate).'</span>' : '-', $resultRateCount));
                    ?> </font></tr>
                <?php
                $zayavkaView->hr5();
...
// подсчет суммы и количества учтенных оценок, и их макс и мин.значения
        ...
        private function subcat_num_to_name($subcat) {
            switch ($subcat) {
                ...
                case 3: $nomination = "Отчёт";
                    $id = "report";
                    break;
                case 4: $nomination = "Лучший поход 2018";
                    $id = "";
                    break;
                default: $nomination = "";
                    $id = "";
        ...
        private function getRefereeRates($class) {
            $q = "SELECT status, class, id FROM $this->db_table_name WHERE status = '1'";
            if ($class !== '') {
                $q .= " AND class = '$class'";
            }
            ...
            for ($i = 0; $i < mysql_num_rows($result); $i++) {
                ...
                $result4 = mysql_query("SELECT difficult, org, otch FROM $this->ref_voting_db_name WHERE id_z = $array[id]");
