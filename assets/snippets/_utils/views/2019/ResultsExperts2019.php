...
// эти параметры меняются год от года
        ...
        $voting_db_name = 'way_voting_expert_2019';
		$voting_db_name2 = 'way_voting_expert_2019_2';
...
// выводим очередную строку таблицы
				...
				$ratesExp =	$this->getExpertRatesTable($subcat, $voting_db_name2, $idz);
				?> 
				<table style="border-collapse:collapse">
				<tr><td>
				<?php
				echo "<br>";
				echo "<td><b>оценки экспертов в Финале:</b></td><td>";
				...
				$ratesExp =	$this->getExpertRatesTable($subcat, $voting_db_name, $idz);
				?> 
				<table style="border-collapse:collapse">
				<tr>
				<?php
				
				echo "<td><b>оценки экспертов в Отборе:</b></td><td>";
				 for ($t = 0; $t < count($ratesExp); $t++) {
					 ?>
					 <td style="border: 1px solid black;">
					  <?php
...
//        echo '<b>' . $array['id'] . '</b>' . ' ' . $inf. ' ' .$inf_count. $inf/($inf_count>0?$inf_count:1)."<br>";
		...
		function getExpertRatesTable($subcat, $voting_db_name, $idz) {
			$result5 = mysql_query("SELECT informative, autonome, unusual, children, unfortun FROM $voting_db_name WHERE idz = $idz");
			$ratesExp = Array();
			$s = 0;
