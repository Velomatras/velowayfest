...
и походы были совершенны полностью или частично в 2024г (для класса "длительные велопоходы" и "велопутешествия" также 2023г).</i><br><br>
        ...
        <table width="100%"  border="0" cellspacing="2" cellpadding="0">
            ...
            $zayavkaView->headerType1_refResults('Средняя по 5/<br>Кол-во номинаций<br>/Средняя по судейским');
			$zayavkaView->hr5();
            $place = 1;
			...
			foreach ($rates as $value) {
				...
				if ($maxWinter < $value['winter']) $maxWinter = $value['winter'];
            ...
            foreach ($rates as $value) {
				...
				$zayavkaList = new zayavkaList($zayavka);
                $rate_sud = $value['rate_sud']; //средний балл по судейским
				
				$class = $value['class']; //класс похода
				
				
                if ($class == 'ПВД') { $class1++;
				$place_class = $class1;
				}
				else if ($class == 'Короткие велопоходы'){ $class2++;
				$place_class = $class2;
				}
				else if ($class == 'Велопоходы'){ $class3++;
				$place_class = $class3;
				}
				else if ($class == 'Длинные велопоходы'){ $class4++;
				$place_class = $class4;
				}
                else if ($class == 'Велопутешествия'){ $class5++;
				$place_class = $class5;
				}
...
//if ($idz == 86) continue; Не учитываем поход Янгировой!!!
				...
				if ($resultRate < $WIN){
					...
					$resultRateCount, sprintf("%.2f", $rate_sud)));
				...
				echo "<td><b>оценки за номинации:</b></td>";
					 ...
					 //Зимний
					 if (($value['max2'] <= $resultWinter) && $count_max < 2){
					
						 $count_max++;
						 if ($value['max2'] == $resultWinter) $flag = 1; 
					 ?>
					 ...
					 <td style="border: 1px solid black;" align="center">
					  ...
					  if ($resultWinter >= $maxWinter) echo "<b>";
					  if ($resultWinter != 0) echo sprintf("%.2f", $resultWinter);
					 else echo "-";
					 if ($resultWinter >= $maxWinter) echo "</b>";
					 echo "</td>";
					 ...
					 //Видео
					  if (($value['max2'] <= $resultVideo) && $count_max < 2 && $resultVideo == $value['best_video']){
					 ...
					 //Фильм
					 if (($value['max2'] <= $resultMovie) && $count_max < 2 && $resultMovie == $value['best_video']){
					 ...
					 //Лекция
					 if (($value['max2'] <= $resultLecture) && $count_max < 2 && $resultLecture == $value['best_video']){
					
						 $count_max++;
						 if ($value['max2'] == $resultLecture) $flag = 1; 
					 ?>
					 ...
					 <td style="border: 1px solid black;" align="center">
					  ...
					  if ($resultLecture >= $maxLecture) echo "<b>";
...
rate_unf, rate_foto_1, rate_foto_2, rate_foto_3, rate_video, rate_videoLong, rate_exciting, class, rate_winter, rate_lecture FROM $db_table_name WHERE status = '1'";
			...
			for ($i = 0; $i < mysql_num_rows($result); $i++) {
				...
				$sud_summ = 0; // сумма оценок за судейских номинации
				...
				if ( $array[16] != 0) {
					$best_summ = $best_summ + $array[16];
					$best_cont++;
					$nosud_count++; 
					$nosud_summ['winter'] = $array[16];
				}
				...
				if ( $array[12] != 0 || $array[13] != 0 || $array[17] != 0) {
					$best_video = max($array[12],$array[13],$array[17]);
					$best_summ = $best_summ + $best_video;
					$best_cont++;
					$nosud_count++; 
					$nosud_summ['best_video'] = $best_video;
				}
				...
				//}
				//if ( $array[17] != 0) {
				//	$best_summ = $best_summ + $array[17];
				//	$best_cont++;
				//	$nosud_count++; 
				//	$nosud_summ['lecture'] = $array[17];
				//}
				...
				if ($best_cont != 0) $best_rate = ($sud_summ + $max1 + $max2)/5; //балл за Гран-При
				$rate_sud = $sud_summ/3; //средний балл по судейским номинациям
