...
class ResultsClassKPohod2024 {
	
	private $class_pohod = NULL;
	private $db_table_name = NULL;
	private $WIN = 0;
	
	public function __construct($class_pohod, $table, $WIN) {
		
		$this->class_pohod = $class_pohod;
				...
				echo "<td><b>оценки за номинации:</b></td>";
					...
					// echo "+" . $value['max1'] . $value['max2'] . $value['max3'] . $value['rate'];
					 ...
					 if ($resultWin >= $value['max3'] && $flag < 3){
						  if (($resultWin == $value['max3']) && ($yellow > 1)) {
							 $yellow = $yellow - 1;
						 ?>
					 					 <td style="border: 1px solid black; background: yellow;" align="center">
					  <?php
					 ...
					 if ($resultLecture >= $value['max3'] && $flag < 3){
						  if (($resultLecture == $value['max3']) && ($yellow > 1)) {
							 $yellow = $yellow - 1;
						 ?>
					 					 <td style="border: 1px solid black; background: yellow;" align="center">
					  <?php
