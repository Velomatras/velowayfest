				...
				echo "<td><b>оценки за номинации:</b></td>";
					...
					// echo "+" . $value['max1'] . $value['max2'] . $value['max3'] . $value['rate'];
					 ...
					 echo "</td>";
					  ...
					  if ($resultWin >= $value['max3'] && $flag < 3){
						 ?>
					 					 <td style="border: 1px solid black; background: yellow;" align="center">
					  <?php
					  $flag++;
					 ...
					 <td style="border: 1px solid black;" align="center">
					  ...
					  if ($resultWin != 0) echo sprintf("%.2f", $resultWin);
					 else echo "-";
					 echo "</td>";
					 ...
					 echo "</td>";
					  ...
					  if ($resultLecture >= $value['max3'] && $flag < 3){
						 ?>
					 					 <td style="border: 1px solid black; background: yellow;" align="center">
					  <?php
					  $flag++;
					 ...
					 <td style="border: 1px solid black;" align="center">
					  ...
					  if ($resultLecture != 0) echo sprintf("%.2f", $resultLecture);
					 else echo "-";
					 echo "</td>";
...
rate_unf, rate_foto_1, rate_foto_2, rate_foto_3, rate_video, rate_videoLong, rate_exciting, rate_winter, rate_lecture FROM $db_table_name WHERE status = '1'";
$q2 = "SELECT id, rate_diff, rate_org, rate_otch, rate_inf, rate_aut, rate_chi, rate_unu, 
rate_unf, rate_foto_1, rate_foto_2, rate_foto_3, rate_video, rate_videoLong, rate_exciting, rate_winter, rate_lecture FROM $db_table_name WHERE status = '-2'";
			...
			$max3 = 0;
				...
				$diff = self::delta($array[1],1);
				$org = self::delta($array[2],2);
				$otch = self::delta($array[3],3);
				$inf = self::delta($array[4],4);
				$aut = self::delta($array[5],5);
				$chi = self::delta($array[6],6);
				$unu = self::delta($array[7],7);
				$unf = self::delta($array[8],8);
				$video = self::delta($array[12],12);
				$movie = self::delta($array[13],13);
				$exciting = self::delta($array[14],14);
				$winter = self::delta($array[15],15);
				$lecture = self::delta($array[16],16);
				...
				if ( $array[1] != 0) {
					...
					list($max1, $max2, $max3) = $this->max3_count($diff, $max1, $max2, $max3);                     				   				 
					$best_cont++;
				}
				if ( $array[2] != 0) {
					$best_summ = $best_summ + $array[2];
					list($max1, $max2, $max3) = $this->max3_count($org, $max1, $max2, $max3); 
					$best_cont++;
				}
				if ( $array[3] != 0) {
					$best_summ = $best_summ + $array[3];
					list($max1, $max2, $max3) = $this->max3_count($otch, $max1, $max2, $max3); 
					$best_cont++;
				}
				if ( $array[4] != 0) {
					$best_summ = $best_summ + $array[4];
					list($max1, $max2, $max3) = $this->max3_count($inf, $max1, $max2, $max3); 
					$best_cont++;
				}
				if ( $array[5] != 0) {
					$best_summ = $best_summ + $array[5];
					list($max1, $max2, $max3) = $this->max3_count($aut, $max1, $max2, $max3); 
					$best_cont++;
				}
				if ( $array[6] != 0) {
					$best_summ = $best_summ + $array[6];
					list($max1, $max2, $max3) = $this->max3_count($chi, $max1, $max2, $max3); 
					$best_cont++;
				}
				if ( $array[7] != 0) {
					$best_summ = $best_summ + $array[7];
					list($max1, $max2, $max3) = $this->max3_count($unu, $max1, $max2, $max3); 
					$best_cont++;
				}
				if ( $array[8] != 0) {
					$best_summ = $best_summ + $array[8];
					list($max1, $max2, $max3) = $this->max3_count($unf, $max1, $max2, $max3); 
					$best_cont++;
				}
				...
				if ( $array[12] != 0 || $array[13] != 0 || $array[16] != 0) {
					$best_video = max($array[12],$array[13],$array[16]);
					list($max1, $max2, $max3) = $this->max3_count($best_video, $max1, $max2, $max3); 
					$best_summ = $best_summ + $best_video;
					$best_cont++;
				}
				
				//if ( $array[12] != 0) {
				//	$best_summ = $best_summ + $array[12];
				//	list($max1, $max2, $max3) = $this->max3_count($video, $max1, $max2, $max3); 
				//	$best_cont++;
				//}
				//if ( $array[13] != 0) {
				//	$best_summ = $best_summ + $array[13];
				//	list($max1, $max2, $max3) = $this->max3_count($movie, $max1, $max2, $max3); 
				//	$best_cont++;
				//}
				if ( $array[14] != 0) {
					$best_summ = $best_summ + $array[14];
					list($max1, $max2, $max3) = $this->max3_count($exciting, $max1, $max2, $max3); 
					$best_cont++;
				}
...
//добавление несущественного малого значения в результат, чтоб не иметь равные значения
static public function delta($par, $delta){
	if (!empty($par)) $sum = $par + ($delta/1000000);
	else $sum = $par;
	return $sum;
	}
