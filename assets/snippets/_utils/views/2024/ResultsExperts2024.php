...
const RATE_WIN_INF = 7;
const RATE_WIN_AUT = 7; 
const RATE_WIN_UNU = 7; 
const RATE_WIN_CHI = 7; 
const RATE_WIN_UNF = 7;
const RATE_WIN_WIN = 7; 
...
function showConstant($subcat) {
	...
	if ($subcat == 1) $ball_sub = self::RATE_WIN_INF;
		...
		if ($subcat == 6) $ball_sub = self::RATE_WIN_WIN;
		return $ball_sub;
}
...
// эти параметры меняются год от года
        ...
        $voting_db_name = 'way_voting_expert_2024';
		$voting_db_name2 = 'way_voting_expert_2024_2';
        $zayavka_Info_Url = 'archive/participant-2024.html';
        $zayavka = new Zayavka2024;

        $subcat = intval($_GET["subcat"]);
        if ($subcat < 1 || $subcat > 6) {
            return;
        }
...
// Заголовок страницы
        ...
        <table width="100%"  border="0" cellspacing="2" cellpadding="0">
            ...
            $zayavkaView->headerType1_refResults('Средняя оценка Финала/<br>Средняя оценка Отбора/<br>Число оценок');
			$zayavkaView->hr5();
            $place = 1;
			$preRate = 0;
            $preSum = 0; 
 ...
 ?>
<tr><td></td><td align="center" colspan="3" class="cell_yellow2" ><font color="green"><b>ТОПОВЫЕ ОТЧЁТЫ ПОЛУЧИВШИЕ ВЫСОКУЮ ОЦЕНКУ!</b></font></td></tr>
...
$zayavkaView->hr5();
            ...
            foreach ($rates_2 as $value) {
                ...
                if (!$resultRate || $resultRate < $ball_sub) {
...
// записываем в библиотеку походов судейские оценки и места занятые походом
			...
			//конец цикла вывода результатов ТОП работ
			
			$zayavkaView->hr5();
?>

<tr><td></td><td align="center" colspan="3" class="cell_yellow2" ><font color="green"><b>ОТЧЁТЫ НЕ ПОЛУЧИВШИЕ ВЫСОКУЮ ОЦЕНКУ!</b></font></td></tr>
			...
			//Цикл вывода результатов финального этапа НЕТОПОВЫЕ работы
            foreach ($rates_2 as $value) {
                $idz = $value['idz'];
                $resultRate = $value['sortField'] > 0 ? $value['sortField'] : 0;
                if (!$resultRate || $resultRate >= $ball_sub) {
...
// записываем в библиотеку походов судейские оценки и места занятые походом
			...
			//конец цикла вывода результатов НЕТОПОВЫХ работ
        ...
        private function subcat_num_to_name($subcat) {
            if ($subcat == 1)
                $valueName = 'Самый познавательный поход';
            if ($subcat == 2)
            ...
            if ($subcat == 6)
				$valueName = 'Самый зимний велопоход';
...
// подсчет суммы и количества учтенных оценок
        ...
        private function getExpertsRates($zayavka, $voting_db_name) {
            ...
            foreach ($zayavkaList->list as $array) {
                $inf = $auto = $unus = $child = $unfor = $winter = 0;
                $inf_count = $auto_count = $unus_count = $child_count = $unfor_count = $winter_count = 0;
                ...
                foreach ($zayavkaVotes as $vote) {
					...
					list($winter, $winter_count) = $this->count_rate_sum($vote['winter'], $winter, $winter_count);
					$infX = $vote['informative'];
					$autX = $vote['autonome'];
					$unuX = $vote['unusual'];
					$chiX = $vote['children'];
					$unfX = $vote['unfortun'];
					$winX = $vote['winter'];
					 ...
					 //if ($infX !== '-') $dispInf[] = array('idz' => $infX);
					//  if ($infX !== '-') $disp_inf[] = array('idz' => $infX);
					//if (!empty($autX)) $disp_aut = array('auto' => $vote['autonome']);
					//if (!empty($unuX)) $disp_unus = array('unus' => $vote['unusual']);
					//if (!empty($chiX)) $disp_child = array('child' => $vote['children']);
					//if (!empty($unfX)) $disp_unfor = array('unfor' => $vote['unfortun']);
					$s++;
                }
				list($ratesExp, $namesExp) = $this->getExpertRatesTable($this->subcat, $voting_db_name, $array['id']);
				$disp_inf = $this->dispers($dispInf);
				$disp_auto = $this->dispers($disp_auto);
				$disp_unus = $this->dispers($disp_unus);
				$disp_child = $this->dispers($disp_child);
				$disp_unfor = $this->dispers($disp_unfor);
				$disp_winter = $this->dispers($disp_winter);
				//$disp = $this->redDisp($ratesExp);
				$disp = $this->dispers($ratesExp);
				...
				$winter = $this->calc_avg_rate($winter, $winter_count);
// формируем массив с оценками
                $rates[] = array(
					...
					'winterCount' => $winter_count,
					'disp_inf' => $disp_inf,
					'disp_auto' => $disp_auto,
					'disp_unus' => $disp_unus,
					'disp_child' => $disp_child,
					'disp_unfor' => $disp_unfor,
					'disp_winter' => $disp_winter,
					'disp' => $disp
                );
//        echo '<b>' . $array['id'] . '</b>' . ' ' . $inf. ' ' .$inf_count. $inf/($inf_count>0?$inf_count:1)."<br>";
		...
		private function getExpertsRates2($zayavka, $voting_db_name) {
            ...
            foreach ($zayavkaList->list as $array) {
				...
				list($ratesExp, $namesExp) = $this->getExpertRatesTable($this->subcat, $voting_db_name, $array['id']);
				$disp_inf = $this->dispers($inf);
				$disp_auto = $this->dispers($auto);
				$disp_unus = $this->dispers($unus);
				$disp_child = $this->dispers($child);
				$disp_unfor = $this->dispers($unfor);
				$disp_winter = $this->dispers($winter);
				$disp = $this->dispers($ratesExp);
        ...
        function sortRates($subcat, &$rates) {
            ...
            foreach ($rates as &$rate) {
                ...
                if ($subcat == 1) {
					...
					$rate["disp"] = $rate["disp_inf"];
                } elseif ($subcat == 2) {
                    $rate["sortField"] = $rate["auto"];
					$rate["summ"] = $rate["autoCount"];
					$rate["disp"] = $rate["disp_auto"];
                } elseif ($subcat == 3) {
                    $rate["sortField"] = $rate["unus"];
					$rate["summ"] = $rate["unusCount"];
					$rate["disp"] = $rate["disp_unus"];
                } elseif ($subcat == 4) {
                    $rate["sortField"] = $rate["child"];
					$rate["summ"] = $rate["childCount"];
					$rate["disp"] = $rate["disp_child"];
                } elseif ($subcat == 5) {
                    $rate["sortField"] = $rate["unfor"];
					$rate["summ"] = $rate["unforCount"];
					$rate["disp"] = $rate["disp_unfor"];
                }  elseif ($subcat == 6) {
                    $rate["sortField"] = $rate["winter"];
					$rate["summ"] = $rate["winterCount"];
					$rate["disp"] = $rate["disp_winter"];
                }
...
return (float) sqrt($var)/sqrt($size);
		...
		function getExpertRatesTable($subcat, $voting_db_name, $idz) {
			$result5 = mysql_query("SELECT informative, autonome, unusual, children, unfortun, userid, winter FROM $voting_db_name WHERE idz = $idz");
			$ratesExp = Array();
			$namesExp = Array();
...
if ($subcat == 6) $ratesExp[$s++] = $array5[6];
