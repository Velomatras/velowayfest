...
class ResultsReferee2024 {

    private $ref_voting_db_name = 'way_voting_referee_2024';
	private $ref_voting_db_name2 = 'way_voting_referee_2024_2';
    private $db_table_name = 'way2024';

//ВАЖНО! Все константы RATE_WIN необходимо продублировать из сниппета votingRef-2024!
	
//Короткие велопоходы
const RATE_WIN_DIFF_1 = 7;
const RATE_WIN_ORG_1 = 7;
const RATE_WIN_OTCH_1 = 7;

//Велопоходы
const RATE_WIN_DIFF_2 = 7;
const RATE_WIN_ORG_2 = 7;
const RATE_WIN_OTCH_2 = 7.0;

//Длинные велопоходы
const RATE_WIN_DIFF_3 = 7;
const RATE_WIN_ORG_3 = 7;
const RATE_WIN_OTCH_3 = 7;

//Велопутешествия
const RATE_WIN_DIFF_4 = 7;
const RATE_WIN_ORG_4 = 7;
const RATE_WIN_OTCH_4 = 7;

//Самый сложный поход Года
const RATE_WIN_DIFF_20 = 8;
const RATE_WIN_ORG_20 = 8;
const RATE_WIN_OTCH_20 = 8;											   
...
function showConstant($cat) {
	$ball = Array(); //Массив оценок
        ...
        switch ($cat) {
		   ...
		   case 20:
			$ball[$cat]['diff'] = self::RATE_WIN_DIFF_20;
			$ball[$cat]['org'] = self::RATE_WIN_ORG_20;
			$ball[$cat]['otch'] = self::RATE_WIN_OTCH_20;
			break;											
			default: 
			$ball[$cat]['diff'] = 0;
			$ball[$cat]['org'] = 0;
...
// Заголовок страницы
        ...
        <div style="padding: 10px; margin: 10px; width: 900px; background-color:#FFFFD7; font-size:16px; font-weight:bold">
			...
			<?php
			$bestDiff = "Самый сложный поход Года";
			if ( !is_null($class)) echo "<i>класс:</i> $class";
            else echo "<i>по всем классам в абсолюте</i>"
			?>																   
        </div>
		<i>* В случае равенства средних оценок при распределении мест, более высокое место получает отчёт с наибольшим количеством оценок. 
Если число оценок также равное, то побеждает отчёт с наименьшей дисперсией оценок.  </i>
<hr>
...
$zayavkaView->hr5();
            foreach ($rates2 as $rate2) {
				...
				$dispRate = $rate2['dispField'];
				
				// цикл ТОПовых
				if ($resultRate >= $ball_sub){
...
// записываем в библиотеку походов судейские оценки и места занятые походом
			...
			//конец цикла вывода результатов ТОПОВЫХ работ
         ...
         function cat_num_to_class($cat) {
            switch ($cat) {
                ...
                case 2: $class = "Велопоходы";
                    break;
                case 3: $class = "Длинные велопоходы";
                    break;
                case 4: $class = "Велопутешествия";
                    break;
                case 21: $class = "";    //если надо вывести абсолют
                    break;
				case 20: $class = null;    //"Лучший поход";
                    break;	
...
// считаем суммы и количество учтенных оценок, и их макс и мин.значения
				...
				$diff_d = $this->dispers($diff_disp);
				$org_d = $this->dispers($org_disp);
				$otch_d = $this->dispers($otch_disp);
// формируем массив с оценками
		...
		else $result5 = mysql_query("SELECT difficult, org, otch, id_ref FROM $ref_voting_db_name WHERE id_z = $idz");
				
			//$q = "SELECT difficult, org, otch, id_ref FROM $ref_voting_db_name WHERE id_z = $idz AND class = '$class1'";
			 //$q = "SELECT difficult, org, otch, id_ref FROM $ref_voting_db_name WHERE id_z = $idz";
			//if (!is_null($class)) {
            //    $q .= " AND class = '$class'";
            //}
         ...
         function sortRates($subcat, &$rates) {
            ...
            function sort_func($a, $b) {
                if ($a['sortField'] == $b['sortField']) {
					if ($a['summ'] == $b['summ']) 
					{ if ($a['dispField'] == $b['dispField'])
						return 0;
					else return ($a['dispField'] > $b['dispField']) ? 1 : -1;
					}					
