...
class ZayavkaEditor2021
{
    const ZAYAVKA_CLASS_NAME = 'Zayavka2021';

    public function ZayavkaEditor($zayavka, $moderatorId, $idz, $regOff)
    {
...
// загружаем фотографии
    ...
    private function ZayavkaEditorEdit($zayavka, $idz, $regOff)
    {
        ?>
   ...
   .colortext {
    background-color: #fbeefd; /* Цвет фона */
        ...
        GUIHelpers::setHiddenField('idz', $idz);
        GUIHelpers::setHiddenField('caller', URLWork::getCurURL());
		//Выбор статуса заявки для юзеров без прав модератора запрещён!
				if ( $regOff <= 3 ){
        ?>
                ...
                <br>
                    <?php 
					//Выбор номинаций запрещён если этап голосования начался
        ...
        <?= RegionWork::populateRegionSelectList(RegionWork::unpack($zayavka->data['region'])); ?>
            ...
            <script type="text/javascript">
                $('#add-quote').click(function () {
                    ...
                    $('#quote-block').after(linkBlock2);
                    return false;
                });
            </script>
			<?php
			}
			else echo "<font color=\"red\">Внимание! Редактор цитат недоступен, так как этап голосований начался!</font>";
			echo "<br>";
			?>
            <br />
			...
			<label for="social">Ссылка на контакт в соцсети</label><br>
                <input name="social" id="social" type="text" size="50" value="<?= $zayavka->data['social'] ?>" />
				<br><br>
			<label for="social">Телефон автора отчёта:</label><br>
                <input name="tel" id="tel" type="text" size="50" value="<?= $zayavka->data['tel'] ?>" />
               <br><br>
            ...
            <br><br>
            <label for="image">Картинка, характеризующая поход. Загрузить файл (jpg, png)</label>
            <br>
