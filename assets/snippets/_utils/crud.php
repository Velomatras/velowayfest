...
//$mysqli = $connect->getConnect();
 ...
 $mysqli = $ucon->getMysqli();  
    ...
    private function prepareSQLQueryCondition($conditions, $conditionWithLike = false) {
		//global out_vote_selector;
		require_once 'sql_queries.php';
		//global get_sql_search_str_for_one_field;
        //global get_sql_search_str_for_one_field_strict;
...
//$dbase = 'u1561280_veloway-test';
        ...
        if ($result === FALSE) {
           // throw new Exception("MYSQL error: " . mysqli_error() . " Query: " . $q);
			throw new Exception (printf("Сообщение ошибки: %s\n", $this->sql2->error . " Query: " . $q));
        }
        return $result;
    }
