@if ($paginator->hasPages())
    <nav role="navigation" aria-label="{{ __('Pagination Navigation') }}" class="flex items-center justify-between">
        ...
        <div class="hidden sm:flex-1 sm:flex sm:items-center sm:justify-between fs-5">
            <div>
                <span class="text-sm text-gray-700 leading-5 p-1 ">
                    {!! __('Страница ') !!}
						{{$paginator->currentPage()}}
						из
						{{$paginator->lastPage()}}<br>
					<!--{!! __('Показаны записи ') !!}	
                    ...
                    @if ($paginator->onFirstPage())
                        <span aria-disabled="true" aria-label="{{ __('pagination.previous') }}">
                            <span class="relative inline-flex items-center px-2 py-2 text-sm font-medium text-gray-500 bg-white border border-gray-300 cursor-default rounded-l-md leading-5" aria-hidden="true">
                                <svg class="w-5 h-5" width="60" fill="currentColor" viewBox="0 0 20 20">
